...
async function main() {
  ...
  const tx = await tokentransferor.allowlistDestinationChain(destinationChain, true);
  const receipt = await tx.wait();
//   const address = await registry.account(implementation, chainID, tokenAddress, tokenId, salt)
  
  if(receipt.status){
   console.log(`Sepolia destination has been allowed`);
  }
   else{
    console.log("allow Destination process has failed");
  }
